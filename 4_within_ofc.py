...
# this script more carefully assesses the effect of granularity on timescale and latency
# using the large ventral frontal cortex dataset collected by Fred Stoll
...
# restrict to monkey OFC data, assign granularity based on area

fred_data = filt_data[filt_data.name=='stoll']

ofc_lai_vl = pd.DataFrame()

for area_ in ['AI','a11ml','a12l','a12m','a12o','a12r','a13l','a13m']:
    
    ofc_lai_vl = pd.concat((ofc_lai_vl,fred_data[fred_data.area==area_]))

ofc_lai_vl['area'] = ofc_lai_vl['area'].str.replace('11ml','11m/l')
...
ofc_lai_vl2['area'] = ofc_lai_vl2['area'].str.replace('11ml','11m/l')
...
sns.barplot(ax=axs,data=counts,x='area',y='size',hue='granularity',palette='Set2',dodge=False) # number successfully fit units
sns.barplot(ax=axs,data=counts2,x='area',y='size',hue='granularity',palette='Set2',alpha=0.5,dodge=False) # total units collected
...
# spatial plots of timescale, not shown
